...
class RopaRecord(Base):
    ...
    rejected_by_id: Mapped[Optional[int]] = mapped_column("rejected_by", Integer, ForeignKey("users.id"), nullable=True)
    ...
    edited_by_id: Mapped[Optional[int]] = mapped_column("edited_by", Integer, ForeignKey("users.id"), nullable=True)
    ...
    rejected_by = relationship("User", foreign_keys=[rejected_by_id], viewonly=True)
    approver = relationship("User", foreign_keys=[approved_by], back_populates="approved_ropa_records")
    edited_by = relationship("User", foreign_keys=[edited_by_id], viewonly=True)
